...
from sh.metadata import MetadataManager
...
def main(arguments):
	...
	if not metadata_file_path.is_file():
		try:
			...
			print("ERROR: Could not generate metadata.json")
			print(format_exception(None, error, error.__traceback__))
			sys.exit(1)
	else:
		try:
			...
			print("ERROR: Could not open metadata.json")
			print(format_exception(None, error, error.__traceback__))
			sys.exit(1)
	...
	for source_name, source_config in config["sources"].items():
		...
		if remote_name not in download_workers_per_remote:
			if remote_name not in config["remote_configs"]:
				print(f'ERROR: max_concurrent_downloads not configured for remote_name: {source_config["remote_name"]}')
				sys.exit(1)
			download_workers_per_remote[remote_name] = config["remote_configs"][remote_name]["max_concurrent_downloads"]
	...
	try:
		...
		while len(process_manager.get_futures()) > 0:
			...
			try:
				...
				for future in as_completed(process_manager.get_futures(), timeout=10): # On TimeoutError, catch it and simply fall back to while loop for regular exiting check
					...
					if future.cancelled():
						contexts_in_progress[root_context_hash].cancelled = True # Consider the status of a root archive cancelled if any tasks for its members are cancelled
					else:
								...
								case ResultStatus.EXTRACT_NEEDED:
									...
									try:
										extract_archive_file_future = process_manager.submit_extract_task(
											...
											root_context,
											extract_archive_file,
											context, thread_sevenzip, root_context
										)
										contexts_in_progress[root_context_hash].futures.add(extract_archive_file_future)
										contexts_in_progress[root_context_hash].files_to_process.add(context["file_path"])
									except RuntimeError: 
